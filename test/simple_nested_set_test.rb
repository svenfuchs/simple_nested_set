...
class SimpleNestedSetTest < Test::Unit::TestCase
  ...
  attr_reader :root, :child_1, :child_2, :child_2_1, :unrelated_root

  test "Node.leaves returns all the leaves" do
  ...
  test "node.<=>(other) compares the node to the given other node based on their lft column" do
    ...
    assert_equal -1, root <=> child_1
    assert_equal -1, root <=> child_2
    assert_equal -1, root <=> child_2_1

    assert_equal 1, child_1 <=> root
    assert_equal 0, child_1 <=> child_1
    assert_equal -1, child_1 <=> child_2
    assert_equal -1, child_1 <=> child_2_1

    assert_equal 1, child_2 <=> root
    assert_equal 1, child_2 <=> child_1
    assert_equal 0, child_2 <=> child_2
    assert_equal -1, child_2 <=> child_2_1

    assert_equal 1, child_2_1 <=> root
    assert_equal 1, child_2_1 <=> child_1
    assert_equal 1, child_2_1 <=> child_2
    assert_equal 0, child_2_1 <=> child_2_1
  end
  ...
  test "node.ancestor_of?(other) returns true if the node is an ancestor of the given other node, false otherwise" do
    assert !root.ancestor_of?(root)
    assert root.ancestor_of?(child_1)
    assert root.ancestor_of?(child_2)
    assert root.ancestor_of?(child_2_1)
  
    assert !child_1.ancestor_of?(root)
    assert !child_1.ancestor_of?(child_1)
    assert !child_1.ancestor_of?(child_2)
    assert !child_1.ancestor_of?(child_2_1)
  
    assert !child_2.ancestor_of?(root)
    assert !child_2.ancestor_of?(child_1)
    assert !child_2.ancestor_of?(child_2)
    assert child_2.ancestor_of?(child_2_1)
  
    assert !child_2_1.ancestor_of?(root)
    assert !child_2_1.ancestor_of?(child_1)
    assert !child_2_1.ancestor_of?(child_2)
    assert !child_2_1.ancestor_of?(child_2_1)
  end
  ...
  test "node.self_or_ancestor_of? returns true if the node equals the given node or is an ancestor of the given node, false otherwise" do
    assert root.self_or_ancestor_of?(root)
    assert root.self_or_ancestor_of?(child_1)
    assert root.self_or_ancestor_of?(child_2)
    assert root.self_or_ancestor_of?(child_2_1)
  
    assert !child_1.self_or_ancestor_of?(root)
    assert child_1.self_or_ancestor_of?(child_1)
    assert !child_1.self_or_ancestor_of?(child_2)
    assert !child_1.self_or_ancestor_of?(child_2_1)
  
    assert !child_2.self_or_ancestor_of?(root)
    assert !child_2.self_or_ancestor_of?(child_1)
    assert child_2.self_or_ancestor_of?(child_2)
    assert child_2.self_or_ancestor_of?(child_2_1)
  
    assert !child_2_1.self_or_ancestor_of?(root)
    assert !child_2_1.self_or_ancestor_of?(child_1)
    assert !child_2_1.self_or_ancestor_of?(child_2)
    assert child_2_1.self_or_ancestor_of?(child_2_1)
  end

  test "node.descendent_of? returns true if the node is a descendent of the given node, false otherwise" do
    assert !root.descendent_of?(root)
    assert !root.descendent_of?(child_1)
    assert !root.descendent_of?(child_2)
    assert !root.descendent_of?(child_2_1)
  
    assert child_1.descendent_of?(root)
    assert !child_1.descendent_of?(child_1)
    assert !child_1.descendent_of?(child_2)
    assert !child_1.descendent_of?(child_2_1)
  
    assert child_2.descendent_of?(root)
    assert !child_2.descendent_of?(child_1)
    assert !child_2.descendent_of?(child_2)
    assert !child_2.descendent_of?(child_2_1)
  
    assert child_2_1.descendent_of?(root)
    assert !child_2_1.descendent_of?(child_1)
    assert child_2_1.descendent_of?(child_2)
    assert !child_2_1.descendent_of?(child_2_1)
  end
  ...
  test "node.self_or_descendent_of? returns true if the node equals the given node or is a descendent of the given node, false otherwise" do
    assert root.self_or_descendent_of?(root)
    assert !root.self_or_descendent_of?(child_1)
    assert !root.self_or_descendent_of?(child_2)
    assert !root.self_or_descendent_of?(child_2_1)
  
    assert child_1.self_or_descendent_of?(root)
    assert child_1.self_or_descendent_of?(child_1)
    assert !child_1.self_or_descendent_of?(child_2)
    assert !child_1.self_or_descendent_of?(child_2_1)
  
    assert child_2.self_or_descendent_of?(root)
    assert !child_2.self_or_descendent_of?(child_1)
    assert child_2.self_or_descendent_of?(child_2)
    assert !child_2.self_or_descendent_of?(child_2_1)
  
    assert child_2_1.self_or_descendent_of?(root)
    assert !child_2_1.self_or_descendent_of?(child_1)
    assert child_2_1.self_or_descendent_of?(child_2)
    assert child_2_1.self_or_descendent_of?(child_2_1)
  end

  test "node.level returns the node's level" do
    assert_equal 0, root.level
    assert_equal 1, child_1.level
    assert_equal 1, child_2.level
    assert_equal 2, child_2_1.level
  end
  ...
  test "node.update_attributes(:parent_id => nil) makes node a root (as the rightmost root)" do
    child_2_1.update_attributes!(:parent_id => nil)
    assert child_2_1.root?
  ...
  test "node.update_attributes(:left_id => nil) makes the node the leftmost (amongst its siblings)" do
    child_2.update_attributes!(:left_id => nil)
    assert_equal root, child_2.parent
    assert_equal child_1, child_2.right_sibling
  end
  ...
  test "node.update_attributes(:right_id => nil) makes the node the rightmost (amongst its siblings)" do
    child_1.update_attributes!(:right_id => nil)
    assert_equal root, child_1.parent
    assert_equal child_2, child_1.left_sibling
  end
